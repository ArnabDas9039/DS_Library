...
public:
    int x, y, m;
    // (x, y) is the coordinate and m is the (manhattan X) "euclidean" distance from the goal
};
...
public:
    ...
    void gbfs(Node* start){
        Heap<Node*, CompareH> HFrontier;
        Map<Node*, bool> visit;
        int size = 0;

        HFrontier.push(start);
        while(!visit[this->end]){
            Node* value = HFrontier.front();
            HFrontier.pop();
            if(!visit[value]){
    ...
    void ass(Node* start){
        Heap<Node*, CompareG> GFrontier;
        Map<Node*, bool> visit;
        int size = 0;

        GFrontier.push(start);
        while(!visit[this->end]){
            Node* value = GFrontier.front();
            GFrontier.pop();
            if(!visit[value]){
                cout << "(" << value->x << ", " << value->y << ", " << value->m << ") ->";
                size++;
